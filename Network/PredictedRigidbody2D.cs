...
struct Rigidbody2DState : PredictedState
{
    ...
    public Vector2 positionDelta; // delta to get from last to this position
    public Vector2 position;

    public float rotation; // TODO delta rotation?

    public Vector2 velocityDelta; // delta to get from last to this velocity
    ...
    {
        positionDelta = Vector2.Lerp(Vector2.zero, positionDelta, multiplier);
        // TODO if we have have a rotation delta, then scale it here too
        velocityDelta = Vector2.Lerp(Vector2.zero, velocityDelta, multiplier);
    }

    public static Rigidbody2DState Interpolate(Rigidbody2DState a, Rigidbody2DState b, float t)
    {
...
public class PredictedRigidbody2D : NetworkBehaviour
{
    ...
    readonly SortedList<double, Rigidbody2DState> stateHistory = new SortedList<double, Rigidbody2DState>();
    ...
    protected virtual void CreateVisualCopy()
    {
        ...
        PredictedRigidbody2DVisual visualRigidbody = visualCopy.AddComponent<PredictedRigidbody2DVisual>();
    ...
    void ApplyState(Vector3 position, float rotation, Vector3 velocity)
    {
    ...
    void RecordState()
    {
        ...
        {
            Rigidbody2DState last = stateHistory.Values[stateHistory.Count - 1];
    ...
    void ApplyCorrection(Rigidbody2DState corrected, Rigidbody2DState before, Rigidbody2DState after)
    {
        ...
        // Rigidbody2DState newest = stateHistory.Values[stateHistory.Count - 1];
    ...
    void CompareState(double timestamp, Rigidbody2DState state)
    {
        ...
        if (!Prediction.Sample(stateHistory, timestamp, out Rigidbody2DState before, out Rigidbody2DState after, out double t))
        {
            ...
            Rigidbody2DState oldest = stateHistory.Values[0];
            Rigidbody2DState newest = stateHistory.Values[stateHistory.Count - 1];
        ...
        Rigidbody2DState interpolated = Rigidbody2DState.Interpolate(before, after, (float)t);
    ...
    public override void OnDeserialize(NetworkReader reader, bool initialState)
    {
        ...
        CompareState(timestamp, new Rigidbody2DState(timestamp, Vector3.zero, position, rotation, Vector3.zero, velocity));
    }
