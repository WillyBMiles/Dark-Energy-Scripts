...
public class PredictedRigidbody2DVisual : MonoBehaviour
{
    [Tooltip("The predicted rigidbody to follow.")]
    public PredictedRigidbody2D target;
    ...
    void LateUpdate()
    {
        ...
        {
            transform.position = targetRigidbody.position;
            transform.rotation = Quaternion.Euler(0f,0f,targetRigidbody.rotation);
        ...
        transform.rotation = Quaternion.Euler(0f,0f,Mathf.Lerp(transform.eulerAngles.z, targetRigidbody.rotation, rotationInterpolationSpeed * Time.deltaTime));
    }
}
